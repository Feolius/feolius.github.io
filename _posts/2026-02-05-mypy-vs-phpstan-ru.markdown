...
Подробнее об этом можно прочесть [тут](https://phpstan.org/blog/remembering-and-forgetting-returned-values). Дело в
том, что в большинстве случаев разработчик ожидает именно запоминания результатов вызовов. Об этом написано прямо вот
так 
...
```
Вот так будет показана корректная ошибка. Более того, можно поменять дефолтное поведение, выставив параметр
...
## Вывод
Сложно переоценить значимость статического анализатора для языков вроде php и python. В наши дни трудно написать
код, лишенный ошибок, без строгой типизации. По крайней мере, в относительно крупных проектах. Поэтому за последние
