...
Но давайте я все же попробую оправдаться, и пояснить за этот наброс.
...
которые исчезают в рантайме (для php это phpdoc). Они позволяют статическому анализатору и IDE разобраться, в чем
...
это [сложно](https://github.com/python/mypy/issues/4468). Может и так. Но от этого не легче.

## Вывод
Невозможно переоценить значимость статического анализатора для языков вроде php и python. В наши дни написать
