...
На мой взгляд, подход с UoW мог бы корректно работать только в том случае, если бы мы с самого начала обработки 
запроса загрузили в UoW все необходимые объекты в память, и никогда после этого не ходили в бд. Но это попросту невозможно. 
